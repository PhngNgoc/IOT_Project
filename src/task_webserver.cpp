...
namespace
{
    ...
    String tinyml_result = "{\"page\":\"tinyml\",\"status\":\"waiting\"}";
...
void Webserver_sendTinyML(String data)
{
    tinyml_result = data;
...
void connnectWSV()
{
    ...
    server.on("/tinyml", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                  AsyncWebServerResponse *response = request->beginResponse(200, "application/json", tinyml_result);
                  addNoCacheHeaders(response);
                  request->send(response);
              });
    ...
    server.on("/device/config", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                  ...
                  {
                      CORE_IOT_TOKEN = request->getParam("token")->value();
                      CORE_IOT_SENSOR_TOKEN = CORE_IOT_TOKEN;
                  }
                  else if (request->hasParam("sensor_token"))
                  {
                      CORE_IOT_TOKEN = request->getParam("sensor_token")->value();
                      CORE_IOT_SENSOR_TOKEN = CORE_IOT_TOKEN;
                  }
                  if (request->hasParam("relay_token"))
                  {
                      CORE_IOT_RELAY_TOKEN = request->getParam("relay_token")->value();
                  }
                  if (request->hasParam("servo_token"))
                  {
                      CORE_IOT_SERVO_TOKEN = request->getParam("servo_token")->value();
                  }
